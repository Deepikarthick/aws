...
source "amazon-ebs" "ec2_ubuntu_server_inst" {
  ami_name = "sloopstash-ubuntu-server-v1.1.1-ami"
  region = var.region
  ...
  ssh_timeout = "1m"
  tags = {
    Name = "sloopstash-ubuntu-server-v1.1.1-ami"
    Region = var.region
...
build {
  name = "ec2_ubuntu_server_ami"
  sources = ["source.amazon-ebs.ec2_ubuntu_server_inst"]
